...
This document describes the `web_fetch` tool for the CyberCLI.
...
## How to use `web_fetch` with the CyberCLI

To use `web_fetch` with the CyberCLI, provide a natural language prompt that contains URLs. The tool will ask for confirmation before fetching any URLs. Once confirmed, the tool will process URLs through Gemini API's `urlContext`.
