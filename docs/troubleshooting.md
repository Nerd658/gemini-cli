...
- **Q: How do I update CyberCLI to the latest version?**
  - A: If installed globally via npm, update CyberCLI using the command `npm install -g @google/gemini-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are CyberCLI configuration files stored?**
...
- **Error: Command not found (when attempting to run CyberCLI).**
  - **Cause:** CyberCLI is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure CyberCLI installation was successful.
