...
```bash
git clone https://github.com/google-cybercli/cybercli-cli.git # Or your fork's URL
cd cybercli-cli
```
...
To build both the `cybercli` CLI utility and the sandbox container, run `build:all` from the root directory:
...
To start the cybercli CLI from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the cybercli-cli folder you can utilize `npm link path/to/cybercli-cli/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or `alias cybercli="node path/to/cybercli-cli/packages/cli"` to run with `cybercli`
...
The integration tests are designed to validate the end-to-end functionality of the cybercli CLI. They are not run as part of the default `npm run test` command.
...
- Consult [GEMINI.md](https://github.com/google-cybercli/cybercli-cli/blob/main/GEMINI.md) (typically found in the project root) for specific instructions related to AI-assisted development, including conventions for React, comments, and Git usage.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for the cybercli CLI.
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/cybercli.js` within the `packages/cli` directory, pausing execution until a debugger attaches. You can then open `chrome://inspect` in your Chrome browser to connect to the debugger.
...
1.  **Start the cybercli CLI in development mode:**
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop cybercli CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.cybercli/sandbox.Dockerfile` and/or `.cybercli/sandbox.bashrc` under your project settings directory (`.cybercli`) and running `cybercli` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
